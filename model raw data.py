...
name starts with "D_" (daily indicator features) and the forward-looking
...
def safe_read_parquet(path: str) -> pl.DataFrame:
    ...
    try:
        ...
        if "timestamp" in pdf.columns:
            # Fix incomplete timezone by replacing trailing '+' with '+05:30'
            pdf["timestamp"] = pdf["timestamp"].astype(str).str.replace(r"\+$", "+05:30", regex=True)
...
def extract_features(df: pl.DataFrame, *, symbol: str | None) -> pl.DataFrame:
    # Match columns starting with 'D_' or lowercase 'd'
    indicator_cols = [col for col in df.columns if col.lower().startswith("d")]
    if not indicator_cols:
        raise ValueError("No indicator columns starting with 'D_' were found")
