...
def compute_daily_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    close = _ensure_float(df["close"]); high = _ensure_float(df["high"]); low = _ensure_float(df["low"])
    open_ = _ensure_float(df["open"]); volume = _ensure_float(df["volume"]).fillna(0.0)
    ...
    # CMF / ADX
    df["D_cmf20"] = ( ((close - low) - (high - close)) / (high - low).replace(0, np.nan) * volume ).rolling(window=20, min_periods=1).sum() / volume.rolling(window=20, min_periods=1).sum()
    ...
    df["D_pivot"] = pivot; df["D_support1"] = 2 * pivot - high; df["D_resistance1"] = 2 * pivot - low
    rng = high - low
    df["D_support2"] = pivot - rng; df["D_resistance2"] = pivot + rng
...
def build_daily(provider: KiteProvider, config: Config, symbol: str, start: dt.date, end: dt.date,
    ...
    if (requested_covers or data_covers) and not force and not recompute_only:
        return out_pq
...
class Pipeline:
    ...
    def build(self, symbols: Sequence[str], start_date: dt.date, end_date: dt.date, *, force: bool, recompute_only: bool):
        ...
        # Daily build only
        daily_start = start_date - dt.timedelta(days=5)
        daily_end = end_date
        for s in symbols:
            try:
...
def _resolve_requested_dates(start_value: Optional[str], end_value: Optional[str], days_value: Optional[int]) -> Tuple[dt.date, dt.date]:
